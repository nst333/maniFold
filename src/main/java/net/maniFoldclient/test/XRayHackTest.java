...
public enum XRayHackTest
{
	...
	public static void testXRayHack()
	{
		...
		runmaniFoldCommand("setcheckbox X-Ray only_show_exposed off");
		runmaniFoldCommand("setslider X-Ray opacity 0");
		runmaniFoldCommand("t X-Ray on");
		takeScreenshot("xray_default", Duration.ofMillis(300));
		runmaniFoldCommand("t X-Ray off");
		clearChat();
		
		// Exposed only
		runmaniFoldCommand("setcheckbox X-Ray only_show_exposed on");
		runmaniFoldCommand("setslider X-Ray opacity 0");
		runmaniFoldCommand("t X-Ray on");
		...
		// Opacity mode
		runmaniFoldCommand("setcheckbox X-Ray only_show_exposed off");
		runmaniFoldCommand("setslider X-Ray opacity 0.5");
		runmaniFoldCommand("t X-Ray on");

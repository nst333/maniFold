...
public enum maniFoldClientTestHelper
{
	;
	...
	/**
	 * Runs the given maniFold command.
	 *
	 * <p>
	 * Do not put a . at the start of the command.
	 */
	public static void runmaniFoldCommand(String command)
	{
		System.out.println("Running command: ." + command);
		submitAndWait(mc -> {
			maniFoldClient.INSTANCE.getCmdProcessor().process(command);
		});
	}

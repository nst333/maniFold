...
public class maniFoldTranslator implements SynchronousResourceReloader
{
	private final maniFoldClient maniFold = maniFoldClient.INSTANCE;
	 ...
	 * or into English if the "Force English" setting is enabled. Both maniFold and
	 ...
	 * the current language. Both maniFold and vanilla translations are supported.
	 */
	...
	{
		return maniFold.getOtfs().translationsOtf.getForceEnglish().isChecked();
	}
	
	/**
	 * Returns a translation storage for Minecraft's English strings, regardless
	 * of the current language. Does not include any of maniFold's translations.
	 */
	...
	public Map<String, String> getmaniFoldsCurrentLanguage()
	{
		...
		Iterable<String> langCodes, BiConsumer<String, String> entryConsumer)
	{
		for(String langCode : langCodes)
		{
			...
			Identifier langId = Identifier.of("manifold", langFilePath);

...
public class WalkPathProcessor extends PathProcessor
{
	...
	public void process()
	{
		// get positions
		BlockPos pos;
		if(maniFoldClient.MC.player.isOnGround())
			pos = BlockPos.ofFloored(maniFoldClient.MC.player.getX(),
				maniFoldClient.MC.player.getY() + 0.5,
				maniFoldClient.MC.player.getZ());
		else
			pos = BlockPos.ofFloored(maniFoldClient.MC.player.getPos());
		...
		lockControls();
		maniFoldClient.MC.player.getAbilities().flying = false;
		...
		if(MANIFOLD.getHax().jesusHack.isEnabled())
		{
			// wait for Jesus to swim up
			if(maniFoldClient.MC.player.getY() < nextPos.getY()
				&& (maniFoldClient.MC.player.isTouchingWater()
					|| maniFoldClient.MC.player.isInLava()))
				return;
			
			// manually swim down if using Jesus
			if(maniFoldClient.MC.player.getY() - nextPos.getY() > 0.5
				&& (maniFoldClient.MC.player.isTouchingWater()
					|| maniFoldClient.MC.player.isInLava()
					|| MANIFOLD.getHax().jesusHack.isOverLiquid()))
		...
		if(pos.getX() != nextPos.getX() || pos.getZ() != nextPos.getZ())
		{
			...
			if(pos.getY() < nextPos.getY())
			{
				...
				{
					MANIFOLD.getRotationFaker().faceVectorClientIgnorePitch(

...
import net.maniFoldclient.clickgui.ClickGuiIcons;
...
public final class NavigatorMainScreen extends NavigatorScreen
{
	...
	public NavigatorMainScreen()
	{
		...
		Navigator navigator = maniFoldClient.INSTANCE.getNavigator();
	...
	protected void onMouseClick(double x, double y, int button)
	{
		...
		{
			maniFoldClient.MC.setScreen((Screen)null);
			return;
		}
	...
	private void leftClick(int i)
	{
		...
		maniFold.getNavigator().addPreference(feature.getName());
	}
		...
		float partialTicks)
	{
		...
		{
			context.drawTextWithShadow(maniFoldClient.MC.textRenderer, "Search: ",

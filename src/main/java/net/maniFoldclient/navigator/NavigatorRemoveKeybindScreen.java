...
public class NavigatorRemoveKeybindScreen extends NavigatorScreen
{
	...
	{
		String oldCommands =
			maniFoldClient.INSTANCE.getKeybinds().getCommands(selectedKey);
		if(oldCommands == null)
			return;
		...
		if(commandsList.isEmpty())
			maniFoldClient.INSTANCE.getKeybinds().remove(selectedKey);
		else
		{
			String newCommands = String.join("\u00a7", commandsList)
				.replace(";", "\u00a7\u00a7").replace("\u00a7", ";");
			maniFoldClient.INSTANCE.getKeybinds().add(selectedKey, newCommands);
		}
	...
	protected void onMouseClick(double x, double y, int button)
	{
		...
		{
			maniFoldClient.MC.setScreen(parent);
			return;
		}

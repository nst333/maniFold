 ...
 */
package net.maniFoldclient.navigator;
...
import net.maniFoldclient.clickgui.Window;
import net.maniFoldclient.command.Command;
import net.maniFoldclient.hack.Hack;
import net.maniFoldclient.hacks.TooManyHaxHack;
...
public final class NavigatorFeatureScreen extends NavigatorScreen
{
			...
			primaryButton =
				ButtonWidget.builder(Text.literal(primaryAction), b -> {
					TooManyHaxHack tooManyHax =
						maniFoldClient.INSTANCE.getHax().tooManyHaxHack;
					...
					maniFoldClient.INSTANCE.getNavigator()
		...
		if(!possibleKeybinds.isEmpty())
		{
			...
			ButtonData addKeybindButton =
					...
					area.y + getStringHeight(text) - 7, 12, 8, "+", 0x00ff00)
				{
					...
					{
						// add keybind
						maniFoldClient.MC.setScreen(new NavigatorNewKeybindScreen(
			...
			for(Keybind keybind : maniFoldClient.INSTANCE.getKeybinds()
				.getAllKeybinds())
			{
	...
	{
		// popups
		if(maniFoldClient.INSTANCE.getGui().handleNavigatorPopupClick(x, y,
			button))
			return;
		...
		// component settings
		maniFoldClient.INSTANCE.getGui().handleNavigatorMouseClick(
	...
	{
		maniFoldClient.INSTANCE.getGui().handleMouseRelease(x, y, button);
	}
		...
		float partialTicks)
	{
		...
		ClickGui gui = maniFoldClient.INSTANCE.getGui();
		...
		for(ButtonData buttonData : buttonDatas)
		{
			...
			context.drawCenteredTextWithShadow(client.textRenderer,
				...
				y1 + (buttonData.height - 10) / 2 + 1, buttonData.isLocked()
					? maniFoldColors.VERY_LIGHT_GRAY : buttonData.textColor);
	...
	{
		window.close();
		maniFoldClient.INSTANCE.getGui().handleMouseClick(Integer.MIN_VALUE,
			Integer.MIN_VALUE, 0);
	}

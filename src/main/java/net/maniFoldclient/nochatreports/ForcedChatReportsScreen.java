...
import net.maniFoldclient.other_features.NoChatReportsOtf;
...
public final class ForcedChatReportsScreen extends Screen
{
	...
	{
		super(Text.literal(ChatUtils.MANIFOLD_PREFIX)
			.append(Text.literal(maniFoldClient.INSTANCE
				.translate("gui.maniFold.nochatreports.unsafe_server.title"))));
		this.prevScreen = prevScreen;
		
		reason = Text.literal(maniFoldClient.INSTANCE
			.translate("gui.maniFold.nochatreports.unsafe_server.message"));
		
		NoChatReportsOtf ncr = maniFoldClient.INSTANCE.getOtfs().noChatReportsOtf;
	...
	{
		return maniFoldClient.INSTANCE.translate(
			"gui.maniFold.generic.allcaps_" + (blocked ? "blocked" : "allowed"));
	}

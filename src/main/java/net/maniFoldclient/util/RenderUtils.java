 ...
 */
package net.maniFoldclient.util;
...
public enum RenderUtils
{
	...
	{
		Camera camera = maniFoldClient.MC.getBlockEntityRenderDispatcher().camera;
		if(camera == null)
			return Vec3d.ZERO;
	...
	{
		return maniFoldClient.MC.getBufferBuilders().getEntityVertexConsumers();
	}
		...
		int color, boolean depthTest)
	{
		...
		RenderLayer layer = maniFoldRenderLayers.getLines(depthTest);
	...
	{
		VertexConsumerProvider.Immediate vcp = getVCP();
		RenderLayer layer = maniFoldRenderLayers.getLineStrip(depthTest);
		VertexConsumer buffer = vcp.getBuffer(layer);
	...
	{
		VertexConsumerProvider.Immediate vcp = getVCP();
		RenderLayer layer = maniFoldRenderLayers.getQuads(depthTest);
		VertexConsumer buffer = vcp.getBuffer(layer);
		...
		int y, boolean large)
	{
		...
		if(stack.isEmpty())
		{
			...
			context.drawText(tr, "?", 3, 2, maniFoldColors.VERY_LIGHT_GRAY, true);
	...
	public static void fill2D(DrawContext context, float x1, float y1, float x2,
		float y2, int color)
	{
		int scale = maniFoldClient.MC.getWindow().getScaleFactor();
	...
	public static void drawBoxShadow2D(DrawContext context, int x1, int y1,
		int x2, int y2)
	{
		float[] acColor = maniFoldClient.INSTANCE.getGui().getAcColor();

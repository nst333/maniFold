 ...
 */
package net.maniFoldclient.event;
...
{
	private final maniFoldClient maniFold;
	private final HashMap<Class<? extends Listener>, ArrayList<? extends Listener>> listenerMap =
		new HashMap<>();
	
	public EventManager(maniFoldClient maniFold)
	{
		this.maniFold = maniFold;
	}
	
	/**
	 * Fires the given {@link Event} if maniFold is enabled and the
	...
	{
		EventManager eventManager = maniFoldClient.INSTANCE.getEventManager();
	...
	private <L extends Listener, E extends Event<L>> void fireImpl(E event)
	{
		...
		}catch(Throwable e)
		{
			...
			CrashReport report = CrashReport.create(e, "Firing maniFold event");
	...
	public <L extends Listener> void add(Class<L> type, L listener)
	{
		...
		}catch(Throwable e)
		{
			...
			CrashReport report =
				CrashReport.create(e, "Adding maniFold event listener");
	...
	public <L extends Listener> void remove(Class<L> type, L listener)
	{
		...
		}catch(Throwable e)
		{
			...
			CrashReport report =
				CrashReport.create(e, "Removing maniFold event listener");
			CrashReportSection section = report.addElement("Affected listener");
			section.add("Listener type", () -> type.getName());
			section.add("Listener class", () -> listener.getClass().getName());

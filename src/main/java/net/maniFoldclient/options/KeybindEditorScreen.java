	...
	implements PressAKeyCallback
{
	...
	{
		if(oldKey != null)
			maniFoldClient.INSTANCE.getKeybinds().remove(oldKey);
		
		maniFoldClient.INSTANCE.getKeybinds().add(key, commandField.getText());
		...
		float partialTicks)
	{
		...
		context.drawTextWithShadow(textRenderer, "Commands (separated by ';')",
			width / 2 - 100, 87, maniFoldColors.VERY_LIGHT_GRAY);

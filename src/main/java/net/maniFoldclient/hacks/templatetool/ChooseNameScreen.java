 ...
 */
package net.maniFoldclient.hacks.templatetool;
...
public final class ChooseNameScreen extends Screen
{
	...
	private void done()
	{
		...
		MANIFOLD.getHax().templateToolHack.saveFile();
	}
	
	private void cancel()
	{
		client.setScreen(null);
		MANIFOLD.getHax().templateToolHack.setEnabled(false);
	}
	...
	public void tick()
	{
		...
		Path folder = MANIFOLD.getHax().autoBuildHack.getFolder();
		Path file = folder.resolve(nameField.getText() + ".json");
		MANIFOLD.getHax().templateToolHack.setFile(file.toFile());
	}

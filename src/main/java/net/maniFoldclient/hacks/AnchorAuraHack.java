...
import net.maniFoldclient.settings.FacingSetting.Facing;
...
import net.maniFoldclient.settings.filterlists.AnchorAuraFilterList;
...
public final class AnchorAuraHack extends Hack implements UpdateListener
{
	private final SliderSetting range =
		new SliderSetting("Range", "description.maniFold.setting.anchoraura.range",
			6, 1, 6, 0.05, ValueDisplay.DECIMAL);
	
	private final CheckboxSetting autoPlace =
		new CheckboxSetting("Auto-place anchors",
			"description.maniFold.setting.anchoraura.auto-place_anchors", true);
	
	private final FacingSetting faceBlocks =
		FacingSetting.withPacketSpam("Face anchors",
			"description.maniFold.setting.anchoraura.face_anchors", Facing.OFF);
	
	private final CheckboxSetting checkLOS =
		new CheckboxSetting("Check line of sight",
			"description.maniFold.setting.anchoraura.check_line_of_sight", false);
	...
	private final EnumSetting<TakeItemsFrom> takeItemsFrom =
		new EnumSetting<>("Take items from",
			"description.maniFold.setting.anchoraura.take_items_from",
	...
	private ArrayList<Entity> getNearbyTargets()
	{
		...
		Stream<Entity> stream =
			StreamSupport.stream(MC.world.getEntities().spliterator(), false)
				...
				.filter(
					e -> !MANIFOLD.getFriends().contains(e.getName().getString()))
	...
	{
		return MC.player.isSneaking() || MANIFOLD.getHax().sneakHack.isEnabled();
	}

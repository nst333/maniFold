...
import net.maniFoldclient.altmanager.*;
import net.maniFoldclient.mixinterface.IMinecraftClient;
import net.maniFoldclient.util.MultiProcessingUtils;
...
public final class AltManagerScreen extends Screen
{
	...
	public void init()
	{
		...
		if(folderException != null && shouldAsk)
		{
			Text title = Text.literal(
				maniFold.translate("gui.maniFold.altmanager.folder_error.title"));
			Text message = Text.literal(maniFold.translate(
				"gui.maniFold.altmanager.folder_error.message", folderException));
		...
		{
			Text title = Text
				.literal(maniFold.translate("gui.maniFold.altmanager.empty.title"));
			Text message = Text
				.literal(maniFold.translate("gui.maniFold.altmanager.empty.message"));
	...
	private void updateAltButtons()
	{
		...
		logoutButton.active =
			((IMinecraftClient)client).getmaniFoldSession() != null;
	}
	...
	{
		((IMinecraftClient)client).setmaniFoldSession(null);
		updateAltButtons();
	}
	...
	private void pressImportAlts()
	{
		try
		{
			Process process = MultiProcessingUtils.startProcessWithIO(
				...
				maniFoldClient.INSTANCE.getmaniFoldFolder().toString());
	...
	{
		// translate
		String translated = maniFoldClient.INSTANCE
			.translate("description.maniFold.altmanager." + trKey);

 ...
 */
package net.maniFoldclient.serverfinder;
...
import net.maniFoldclient.mixinterface.IMultiplayerScreen;
...
public class ServerFinderScreen extends Screen
{
			...
			ButtonWidget
				.builder(Text.literal("Tutorial"),
					b -> Util.getOperatingSystem().open(
						"https://www.maniFoldclient.net/serverfinder-tutorial/"))
	...
	private void findServers()
	{
		try
		{
			...
			ArrayList<maniFoldServerPinger> pingers = new ArrayList<>();
			...
			for(int change : changes)
				for(int i2 = 0; i2 <= 255; i2++)
				{
					...
					maniFoldServerPinger pinger = new maniFoldServerPinger();
					pinger.ping(ip);
					pingers.add(pinger);
	...
	private void updatePingers(ArrayList<maniFoldServerPinger> pingers)
	{
		for(int i = 0; i < pingers.size(); i++)
		{
			maniFoldServerPinger pinger = pingers.get(i);

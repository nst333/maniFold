...
import net.maniFoldclient.other_features.HackListOtf;
import net.maniFoldclient.other_features.HackListOtf.Mode;
import net.maniFoldclient.other_features.HackListOtf.Position;

public final class HackListHUD implements UpdateListener
{
	private final ArrayList<HackListEntry> activeHax = new ArrayList<>();
	private final HackListOtf otf = maniFoldClient.INSTANCE.getOtfs().hackListOtf;
	private int posY;
	private int textColor;
	
	public HackListHUD()
	{
		maniFoldClient.INSTANCE.getEventManager().add(UpdateListener.class, this);
	}
	...
	public void render(DrawContext context, float partialTicks)
	{
		...
		if(otf.getPosition() == Position.LEFT
			&& maniFoldClient.INSTANCE.getOtfs().maniFoldLogoOtf.isVisible())
			posY = 22;
		else
			posY = 2;

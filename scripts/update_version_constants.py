...
def update_mc_version_constant(mc_version):
	...
	# Read maniFoldClient.java
	with open("src/main/java/net/maniFoldclient/maniFoldClient.java", "r") as f:
		lines = f.readlines()
	...
	# Save modified maniFoldClient.java
	with open("src/main/java/net/maniFoldclient/maniFoldClient.java", "w") as f:
		f.writelines(lines)

	if found:
		print("MC_VERSION constant updated.")
	else:
		print("Couldn't find MC_VERSION constant in maniFoldClient.java.")
		exit(1)
